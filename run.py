...
author = 'MUSTAQIM'
fb_me = 'facebook.com/108111314694312'
github = 'github.com/MSTQM-XD'

def banner():
	os.system('clear')
	logo = (f'# • Author : {author} •')
	play = rich.markdown.Markdown(logo, style='purple')
	rich.console.Console().print(play)
	...
	try:
		...
		while True:
			...
			if rom in(""):
				...
				jalan("\n%s!%s Wajib gunakan akun tumbal ya dek, dilarang akun utama"%(M,O))
				kukis = input("%s# %sCookie %s> %s"%(P,O,M,K))
				if kukis in(""):
					print ("%s%s isi cookie nya woi "%(M,til))
					exit()
				else:
					konverter(kukis)
					masuk(kukis).login()
				...
				ah = rich.markdown.Markdown(tutorial, style='purple')
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			print ('%s•%s 01 %sCrack dari Friend list '%(U,P,O))
			print ('%s•%s 02 %sCrack dari total followers'%(U,P,O))
			print ('%s•%s 03 %sCrack dari reaction post'%(U,P,O))
			print ('%s•%s 04 %sCrack dari komentar post'%(U,P,O))
			print ('%s•%s 05 %sCrack dari member group'%(U,P,O))
